...
class HandlersCache:
    ...
    def get(self, name: str) -> Optional[DatabaseHandler]:
        ...
        with self._lock:
            ...
            if key not in self.handlers or self.handlers[key]["expired_at"] < time():
                return None
            ...
            with self._lock:
                for key in list(self.handlers.keys()):
                    if (
                        ...
                        and sys.getrefcount(self.handlers[key])
                        == 2  # returned ref count is always 1 higher
                    ):
                        try:
                            self.handlers[key].disconnect()
                        except Exception:
...
class IntegrationController:
    ...
    def delete(self, name):
        ...
        if len(active_models) > 0:
            raise Exception(
                f"Unable to drop ml engine with active models: {active_models}"
            )
    ...
    def _get_integration_record_data(self, integration_record, sensitive_info=True):
        ...
        class_type = None
        if integration_module is not None and inspect.isclass(
            integration_module.Handler
        ):
    ...
    def get_all(self, sensitive_info=True):
        integration_records = (
            db.session.query(db.Integration).filter_by(company_id=ctx.company_id).all()
        )
        integration_dict = {}
        for record in integration_records:
            if record is None or record.data is None:
                continue
            integration_dict[record.name] = self._get_integration_record_data(
    ...
    def _make_handler_args(
        self, handler_type: str, connection_data: dict, integration_id: int = None
    ):
        handler_ars = dict(
            connection_data=connection_data, integration_id=integration_id
        )
    ...
    def create_tmp_handler(self, handler_type: str, connection_data: dict) -> object:
        ...
        handler_ars = dict(
            name="tmp_handler", fs_store=fs_store, connection_data=connection_data
        )

        logger.debug(
            "%s.create_tmp_handler: create a client to db of %s type",
            self.__class__.__name__,
            handler_type,
        )
        return DBClient(
            handler_type, self.handler_modules[handler_type].Handler, **handler_ars
        )
    ...
    def get_handler(self, name, case_sensitive=False):
        ...
        if integration_engine not in self.handler_modules:
            raise Exception(
                f"Can't find handler for '{integration_name}' ({integration_engine})"
            )
        ...
        if handler_type == "ml":
            handler_ars["storage_factory"] = FileStorageFactory(
                resource_group=RESOURCE_GROUP.PREDICTOR, sync=True
            )
        ...
        if isinstance(HandlerClass, type) and issubclass(HandlerClass, BaseMLEngine):
            ...
            handler_ars["execution_method"] = getattr(
                self.handler_modules[integration_engine], "execution_method", None
            )
            handler_ars["integration_engine"] = integration_engine
            logger.info(
                "%s.get_handler: create a ML client, params - %s",
                self.__class__.__name__,
                handler_ars,
            )
        ...
        if hasattr(module, "icon_path"):
            ...
            if handler_meta["icon"]["type"] == "svg":
                ...
                with open(str(icon_path), "rb") as f:
                    handler_meta["icon"]["data"] = base64.b64encode(f.read()).decode(
                        "utf-8"
                    )
        # endregion
    ...
    def _load_handler_modules(self):
        ...
        if not os.path.isdir(handlers_path):
            mindsdb_path = Path(
                importlib.util.find_spec("mindsdb").origin
            ).parent.joinpath("mindsdb")
            handlers_path = mindsdb_path.joinpath("integrations/handlers")
        ...
        for handler_dir in handlers_path.iterdir():
            ...
            try:
                handler_module = importlib.import_module(
                    f"mindsdb.integrations.handlers.{handler_folder_name}"
                )
